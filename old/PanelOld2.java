...
public class PanelOld2 extends JPanel implements Runnable, MouseListener,
	...
	void initPlayer() {
		play = new Player(new float[] { 50, 50 }, 18, 20, Color.BLUE);
		panels = new Indie[1];
		// for (int p = 0; p < panels.length; p++) {
	...
	public void mousePressed(MouseEvent me) {
		...
		if (me.getButton() == MouseEvent.BUTTON3) {
			...
			for (Indie p : panels) {
				// panelTouch = p.clickHandle(me.getX(), me.getY());
				if (!p.closed) {
					panelTouch = p.clickHandle(me.getX(), me.getY()) ? true
							: panelTouch;
				}
				// if (p.clickHandle(me.getX(), me.getY())) {
				// panelTouch = true;
				// }
			}
			if (panelTouch) {
	...
	public void mouseDragged(MouseEvent me) {
		...
		if (lastButton == 1) {
			...
			setPath = true;
			tarX = cameraLoc[0] + me.getX();
			tarY = cameraLoc[1] + me.getY();
		}
		draw();
		drwGm();
	}
